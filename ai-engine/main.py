...
async def rag_hybrid_search(payload: HybridSearchRequest):
    """
    Hybrid search with input guardrails (Prompt 8).
    BM25 (keyword) + dense (semantic) fused via Reciprocal Rank Fusion.
    """
    from rag.hybrid_search import hybrid_search
    from rag.retriever import query as dense_query
    from guardrails.rails_engine import get_engine

    loop   = asyncio.get_running_loop()
    engine = get_engine()

    # Input rail — block jailbreak / injection before running retrieval
    ir = await loop.run_in_executor(_executor, lambda: engine.check_input(payload.query))
    if not ir.passed:
        return {
            "blocked":    True,
            "query":      payload.query,
...
@app.post("/rag/ask")
async def rag_ask(payload: AgenticRAGRequest):
    """
    Agentic RAG with NeMo-style Guardrails (Prompt 8).

    Input rails:   check_length → check_jailbreak → check_prompt_injection
                   → check_pii_request → check_off_topic (LLM)
    Output rails:  check_pii_output → check_sensitive_disclosure

    On block: returns HTTP 200 with blocked=True so the frontend renders
    the safe_message instead of crashing.  Risk score is included for logging.

    On pass: returns normal RAG result + guardrails metadata in 'guardrails' key.
    """
    from agents.agentic_rag import ask as rag_ask_fn
    from guardrails.rails_engine import get_engine
    ...
    if not input_result.passed:
        logger.warning(
            ...
            input_result.rail_triggered, input_result.risk_score,
        )
        return {
            "blocked":       True,
            "answer":        input_result.safe_message or "I cannot help with that request.",
            "sources":       [],
            "sub_queries":   [],
            "hops":          0,
            "is_grounded":   False,
    ...
    output_result = await loop.run_in_executor(
        _executor,
        lambda: engine.check_output(result.get("answer", ""), payload.question),
    )
    if not output_result.passed:
        logger.warning(
            "[guardrails] output blocked — rail=%s score=%.2f",
            output_result.rail_triggered, output_result.risk_score,
        )
        result["answer"]     = output_result.safe_message or "The response was filtered."
        result["blocked"]    = True
        result["guardrails"] = output_result.to_dict()
        return result

    result["blocked"]    = False
    result["guardrails"] = {
        "passed":         True,
        "input_latency":  input_result.latency_ms,
        "output_latency": output_result.latency_ms,
        "risk_score":     0.0,
    }
    return result
...
# ─── Guardrails endpoints (Prompt 8) ─────────────────────────────────────────

@app.get("/guardrails/config")
async def guardrails_config():
    """Return active guardrail rails and policies (safe, non-sensitive portion)."""
    from guardrails.rails_engine import _load_config
    cfg = _load_config()
    return {
        "rails":    cfg.get("rails", {}),
        "policies": cfg.get("policies", {}),
        "topics": {
            "allowed_keywords_count": len(cfg.get("topics", {}).get("allowed_keywords", [])),
        },
        "blocked_pattern_counts": {
            k: len(v) for k, v in cfg.get("blocked_patterns", {}).items()
        },
        "output_filter_counts": {
            k: len(v) for k, v in cfg.get("output_filters", {}).items()
        },
    }


@app.post("/guardrails/check")
async def guardrails_check(payload: dict):
    """
    Ad-hoc guardrail check for a given text string.
    Body: {"text": "...", "mode": "input"|"output"}
    Returns GuardrailResult dict.
    """
    from guardrails.rails_engine import get_engine
    ...
    loop   = asyncio.get_running_loop()
    if mode == "output":
        result = await loop.run_in_executor(_executor, lambda: engine.check_output(text))
    else:
